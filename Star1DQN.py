...
# 加载模型
loadMod = './dqn_model_2100(7.92).ckpt'
if os.path.exists(loadMod):
   agent.restore(loadMod)
   print("成功加载模型",loadMod)
...
while episode < max_episode:  # 训练max_episode个回合，test部分不计算入episode数量
    ...
    logger.info('episode:{}    e_greed:{}  train_reward:{} test_reward:{}'.format(
        episode, agent.e_greed, total_reward,eval_reward))
    if eval_reward>eval_reward_max:
        save_path = 'dqn_model_{}({:.2f}).ckpt'.format(episode, eval_reward)
        agent.save(save_path)
        eval_reward_max = eval_reward
# 训练结束，保存模型
save_path = './dqn_model_final.ckpt'
agent.save(save_path)
